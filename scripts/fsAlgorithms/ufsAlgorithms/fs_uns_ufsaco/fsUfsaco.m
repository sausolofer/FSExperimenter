function [out] = fsUfsaco(instances)
% Unsupervised feature selection based on ant colony optimization (UFSACO) 
% is a multivariate filter method based on the ant colony optimization. 
% The UFSACO performs an explicit redundancy analysis and implicit relevance analysis. 
...
ufsaco = javaObject('kfst.featureSelection.filter.unsupervised.UFSACO',...
    sizeSelectedFeatureSubset, initPheromone, numIterations, numAnt, numFeatureOfAnt, evaporationRate, betaParameter, q0_Parameter);
ufsaco.loadDataSet(inputData, int32(numAttributes), int32(numOfClasses));
ufsaco.evaluateFeatures();
subset = ufsaco.getSelectedFeatureSubset();
FeatureValues = ufsaco.getValues();
