...
% Se calcula el score Laplaciano 
 ...
 %ordena los indices con respecto a sus valores
  ...
  out.W = LaplacianScoreRes; %weight of each features
  ...
  out.prf = -1; %smaller the weights are the more relevant (.prf=-1). [.prf, this field specifies whether the larger the weights the more relevant the features (.prf=1),
                %or the smaller the weights, the more relevant (.prf=-1). For “subset selection” algorithms, if
                %(.prf=0), the features in the “fList” are not ordered.]