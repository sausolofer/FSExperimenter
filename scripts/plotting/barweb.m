function handles = barweb(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend, error_sides, legend_type, colors)
...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
    ...
    for i=1:length(barvalues)
       % handles.bars = bar(barvalues(i),char(colors(i)), width,'edgecolor','b', 'linewidth', 2);
        handles.bars = bar(i,barvalues(i),'FaceColor',colors(i,:));
        hold on
    end
	...
	if ~isempty(bw_legend) && ~strcmp(legend_type, 'axis')
		%handles.legend = legend({bw_legend}, 'Location','northoutside','Orientation','horizontal');
        handles.legend = legend(bw_legend, 'Location','northeastoutside','Orientation','vertical');
		%legend boxoff;
	else
		handles.legend = [];
	end
	...
	for i = 1:numbars
		...
		%x = mean(x([1 3],:));
		handles.errors(i) = errorbar(barvalues(:,i), errors(:,i), 'k', 'linestyle', 'none', 'linewidth', 2);
	...
	ylim([0 ymax*1.1]);
	xlim([0.4 numgroups-change_axis+.6]);
